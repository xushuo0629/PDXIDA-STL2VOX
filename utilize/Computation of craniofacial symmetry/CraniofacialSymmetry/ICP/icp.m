...
inp.addOptional( 'iter', 20, @(x)x > 0 && x < 10^5 );
...
inp.addParameter('Weight', @(x)ones(1,length(x)), @(x)isreal(x) && size(x,2) == size(q,2) );
...
for k = 1:arg.iter       
    % Do matching
    switch arg.Matching
        case 'bruteForce',  [match, mindist] = match_bruteForce(q,pt);
        case 'Delaunay',    [match, mindist] = match_Delaunay(q,pt,DT);
        case 'kDtree',      [match, mindist] = match_kDtree(q,pt,kdOBJ);
    end
    ...
    if arg.Extrapolation
        ...
        if k>2 && theta(k+1) < 10 && theta(k) < 10
            ...
            if dv ~= 0
                ...
                switch arg.Matching
                    case 'bruteForce',  [~, mindist] = match_bruteForce(q,pt);
                    case 'Delaunay',    [~, mindist] = match_Delaunay(q,pt,DT);
                    case 'kDtree',      [~, mindist] = match_kDtree(q,pt,kdOBJ);
                end
    ...
    % Break from the cycle if the error is too low
    if abs( ER(k,1) - ER(k+1,1) ) < 0.01 && k > 10, break, end
end
...
if k == arg.iter
    ...
    % Process finished before the end of the number of iterations
    TR = TR(:,:,k+1);
    TT = TT(:,:,k+1);
    ER = ER(1:k+1,1);
    t  = t(k+1,1);
